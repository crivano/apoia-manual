...
1. Na listagem de prompts, localize a coluna de estrelas.
2. Clique sobre as estrelas (mesmo que estejam apagadas) para abrir a caixa de avaliação.
3. Selecione a quantidade de estrelas desejada para registrar sua avaliação. A nota média do prompt será atualizada.

**3. Gerenciamento Automático de Qualidade**

A avaliação por estrelas está diretamente ligada ao status de compartilhamento dos prompts. A Apoia utiliza a nota média para filtrar automaticamente a qualidade dos prompts públicos:

* Rebaixamento Automático: Se um prompt "Público" receber avaliações que façam sua nota média cair para menos de 2.5 estrelas, o sistema o rebaixará automaticamente para o status "Não Listado".
